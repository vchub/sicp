  ...
  (testing "merge-envs infinite loop"
    (let [env (make-env)
      ...
      (eval-f '(def-f! factorial (fn[n] (if (< n 2)
                                          1
                                          (* n (factorial (- n 1)))))))
      (is (= 24 (eval-f '(factorial j))))

      (eval-f '(def-f! make-factorial (fn []
                                        (fn[n] (if (< n 2)
                                          1
                                          (* n (factorial (- n 1))))))))

      (is (= 24 (eval-f '((make-factorial) 4))))
      (is (= 8 (eval-f '((fn[x](* 2 x)) 4))))
      (eval-f '(def-f! fact (make-factorial)))
      (is (= 24 (eval-f '(fact 4))))
      ;; (is (= 8 (eval-f '(def-f! fact (make-factorial)))))
      ))
